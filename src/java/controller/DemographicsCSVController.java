...
/**
 * DemographicsCSVController contains methods in validating the demographics csv
 * file and inserting them into database if valid
 *
 * @author Jesper
 */
public class DemographicsCSVController {
    ...
    /**
     * Constructor of DemographicsCSVController Instantiate DemographicsDAO to
     * be used
     */
    ...
    public ArrayList<Demographics> getUsersByNeighbourHood(String nb) {
        ArrayList<Demographics> demoList = demographicsDAO.retrieveByNeighbourhood(nb);
       /* *********************** Needs reworking
        for (Demographics d : demoList) {
            if (  d.getRole().equals("manager")  ) {
                demoList.remove(d);
            }
        }*/
        return demoList;
